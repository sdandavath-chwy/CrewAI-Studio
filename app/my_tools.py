...
from tools.SnowflakeQueryTool import SnowflakeQueryTool
from tools.S3FileQueryTool import S3FileQueryTool
...
# Custom SnowflakeQueryTool
class MySnowflakeQueryTool(MyTool):
    def __init__(self, tool_id=None, query=None):
        parameters = {
            'query': {'mandatory': True}
        }
        super().__init__(tool_id, 'SnowflakeQueryTool', "A tool that runs a SQL query on Snowflake and returns the result.", parameters, query=query)

    def create_tool(self) -> SnowflakeQueryTool:
        return SnowflakeQueryTool()


# S3FileQueryTool integration
class MyS3FileQueryTool(MyTool):
    def __init__(self, tool_id=None, s3_path=None):
        parameters = {
            's3_path': {'mandatory': True}
        }
        super().__init__(tool_id, 'S3FileQueryTool', "A tool that loads a file from S3 and returns its contents or runs logic on it.", parameters, s3_path=s3_path)

    def create_tool(self) -> S3FileQueryTool:
        return S3FileQueryTool()
